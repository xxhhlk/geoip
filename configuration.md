...
- args：（必须）
  ...
  - wantedList：（可选，数组）需要的类别（文件）。（与 `inputDir` 同时使用，用于筛选需要的类别或文件）
...
```jsonc
{
  ...
  "args": {
    ...
    "wantedList": ["cn", "us", "jp"], // 只需要 ./clash/yaml 目录里文件名去除扩展名后，名为 cn、us、jp 的文件
...
- args：（必须）
  - wantedList：（可选，数组）需要的类别（文件）。（用于筛选需要的类别）
  - onlyIPType：（可选）只处理的 IP 地址类型，值为 `ipv4` 或 `ipv6`。
...
- args：（必须）
  ...
  - inputDir：需要遍历的输入目录（不遍历子目录）。遍历的文件名作为类别名称。（不能与 `name` 和 `uri` 同时使用）
  - wantedList：（可选，数组）需要的类别（文件）。（用于筛选需要的类别或文件）
  - onlyIPType：（可选）只处理的 IP 地址类型，值为 `ipv4` 或 `ipv6`。
...
```jsonc
{
  ...
  "args": {
    "inputDir": "./json",
    "wantedList": ["cn", "us", "jp"], // 只需要 ./json 目录里文件名去除扩展名后，名为 cn、us、jp 的文件
    "onlyIPType": "ipv6", // 只添加 IPv6 地址
...
- args：（必须）
  ...
  - wantedList：（可选）需要的类别（文件）。（用于筛选需要的类别）
  - onlyIPType：（可选）只处理的 IP 地址类型，值为 `ipv4` 或 `ipv6`。
...
- action：（必须）操作类型，值为 `add`（添加 IP 地址）或 `remove`（移除 IP 地址）
- args：（必须）
  - country：（必须）MaxMind GeoLite2 Country CSV location 文件路径（`GeoLite2-Country-Locations-en.csv`），可为本地文件路径或远程 `http`、`https` 文件 URL。
...
- action：（必须）操作类型，值为 `add`（添加 IP 地址）或 `remove`（移除 IP 地址）
- args：（必须）
  - uri：MaxMind GeoLite2 Country mmdb 格式文件路径，可为本地文件路径或远程 `http`、`https` 文件 URL
  - wantedList：（可选）需要的类别（文件）。（用于筛选需要的类别）
  - onlyIPType：（可选）只处理的 IP 地址类型，值为 `ipv4` 或 `ipv6`。
...
```jsonc
{
  ...
  "action": "add", // 添加 IP 地址
  "args": {
    "uri": "https://github.com/Loyalsoldier/geoip/raw/release/GeoLite2-Country.mmdb",
...
`private` 默认添加或移除的 IP / CIDR 地址，见 [private.go](https://github.com/Loyalsoldier/geoip/blob/HEAD/plugin/special/private.go#L16-L36)
...
```jsonc
{
  ...
  "args": {
    ...
    "wantedList": ["cn", "us", "jp"], // 只需要 ./singbox/srs 目录里文件名去除扩展名后，名为 cn、us、jp 的文件
    "onlyIPType": "ipv6" // 只添加 IPv6 地址
  }
}
```
...
```jsonc
{
  ...
  "action": "add", // 添加 IP 地址
  "args": {
    "inputDir": "./surge/cn.txt", // 遍历 ./surge/cn.txt 目录内的所有文件（不遍历子目录）
    "wantedList": ["cn", "us", "jp"], // 只需要 ./surge/cn.txt 目录里文件名去除扩展名后，名为 cn、us、jp 的文件
    "onlyIPType": "ipv6" // 只添加 IPv6 地址
  }
}
```
...
- args：（必须）
  ...
  - wantedList：（可选，数组）需要的类别（文件）。（与 `inputDir` 同时使用，用于筛选需要的类别或文件）
  - onlyIPType：（可选）只处理的 IP 地址类型，值为 `ipv4` 或 `ipv6`
  - removePrefixesInLine：（可选，数组）每一行需要移除的前缀
  - removeSuffixesInLine：（可选，数组）每一行需要移除的后缀
...
```jsonc
{
  ...
  "args": {
    "name": "cn",
    "uri": "./cn.txt", // 读取本地文件 cn.txt 的 IPv4 和 IPv6 地址，并添加到 cn 类别中
    "removePrefixesInLine": ["Host,", "IP-CIDR", "IP-CIDR6"], // 从读取的文件中移除多种不同的行前缀
...
```jsonc
{
  ...
  "action": "add", // 添加 IP 地址
  "args": {
    "inputDir": "./cn.txt", // 遍历 ./surge/cn.txt 目录内的所有文件（不遍历子目录）
    "wantedList": ["cn", "us", "jp"], // 只需要 ./surge/cn.txt 目录里文件名去除扩展名后，名为 cn、us、jp 的文件
    "onlyIPType": "ipv6", // 只添加 IPv6 地址
    "removePrefixesInLine": ["Host,", "IP-CIDR", "IP-CIDR6"], // 从读取的文件中移除多种不同的行前缀
    "removeSuffixesInLine": [",no-resolve"] // 从读取的文件中移除行后缀
...
```jsonc
{
  ...
  "action": "add", // 添加 IP 地址
  "args": {
    "uri": "./geoip.dat", // 读取本地文件 geoip.dat 中的类别、IPv4 和 IPv6 地址
...
```jsonc
{
  ...
  "args": {
    ...
    "outputExtension": ".yaml" // 输出的文件的扩展名为 .yaml
  }
}
```
...
```jsonc
{
  ...
  "action": "output",
  "args": {
    "search": "1.1.1.0/24", // 查询 CIDR 1.1.1.0/24 是否存在于类别 au、cn、us 中，返回由英文逗号连接的类别字符串，如: au,cn
    "searchList": ["au", "cn", "us"]
  }
}
```
...
```jsonc
{
  ...
  "args": {
    ...
    "overwriteList": ["cn", "google"] // 确保 cn、private 类别最后写入
  }
}
```
...
```jsonc
{
  ...
  "args": {
    ...
    "outputExtension": ".conf" // 输出的文件的扩展名为 .conf
  }
}
```
...
- args：（可选）
  ...
  - wantedList：（可选，数组）需要输出的类别
  - onlyIPType：（可选）输出的 IP 地址类型，值为 `ipv4` 或 `ipv6`
  - addPrefixInLine：（可选）给每一行添加的字符串前缀
  - addSuffixInLine：（可选）给每一行添加的字符串后缀
...
- args：（可选）
  ...
  - wantedList：（可选，数组）需要输出的类别
  - onlyIPType：（可选）输出的 IP 地址类型，值为 `ipv4` 或 `ipv6`
  - oneFilePerList：（可选）每个类别输出为一个单独文件，值为 `true` 或 `false`（默认值）
...
```jsonc
{
  ...
  "action": "output",
  "args": {
    "oneFilePerList": true // 每个类别输出为一个文件
  }
}
```
...
```jsonc
{
  ...
  "args": {
    ...
    "wantedList": ["telegram", "google", "cloudflare"], // 只输出 telegram、google、cloudflare
